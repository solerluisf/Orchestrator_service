...
#[async_trait]
impl IServiceConnector for ExecutionConnector {
    ...
    async fn health_check(&self) -> Result<ServiceHealth, OrchestratorError> {
        match self.event_bus.request_reply("execution_service", b"health").await {
            Ok(_) => Ok(ServiceHealth {
                service_id: "execution_service".to_string(),
    ...
    async fn send_command_inner(&self, cmd: ServiceCommand) -> Result<ServiceAck, OrchestratorError> {
        ...
        let response = self.event_bus
            .request_reply("execution_service", &payload)
            .await
