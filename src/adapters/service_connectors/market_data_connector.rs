...
#[async_trait]
impl IServiceConnector for MarketDataConnector {
    ...
    async fn health_check(&self) -> Result<ServiceHealth, OrchestratorError> {
        match self.event_bus.request_reply("market_data_service", b"health").await {
            Ok(_) => Ok(ServiceHealth {
                service_id: "market_data_service".to_string(),
    ...
    async fn send_command(&self, cmd: ServiceCommand) -> Result<ServiceAck, OrchestratorError> {
        ...
        let response = self.event_bus
            .request_reply("market_data_service", &payload)
            .await
