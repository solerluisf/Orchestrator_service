...
pub struct FeatureConnector {
    event_bus: std::sync::Arc<dyn IEventBusPort>,
}

impl FeatureConnector {
...
#[async_trait]
impl IServiceConnector for FeatureConnector {
    ...
    async fn health_check(&self) -> Result<ServiceHealth, OrchestratorError> {
        match self.event_bus.request_reply("feature_service", b"health").await {
            Ok(_) => Ok(ServiceHealth {
                service_id: "feature_service".to_string(),
    ...
    async fn send_command(&self, cmd: ServiceCommand) -> Result<ServiceAck, OrchestratorError> {
        ...
        let response = self.event_bus
            .request_reply("feature_service", &payload)
            .await
