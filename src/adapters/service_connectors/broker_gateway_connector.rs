...
pub struct BrokerGatewayConnector {
    event_bus: std::sync::Arc<dyn IEventBusPort>,
}

impl BrokerGatewayConnector {
...
#[async_trait]
impl IServiceConnector for BrokerGatewayConnector {
    ...
    async fn health_check(&self) -> Result<ServiceHealth, OrchestratorError> {
        match self.event_bus.request_reply("broker_gateway", b"health").await {
    ...
    async fn send_command_inner(&self, cmd: ServiceCommand) -> Result<ServiceAck, OrchestratorError> {
        ...
        let response = self.event_bus
            .request_reply("broker_gateway", &payload)
            .await
